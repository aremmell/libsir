...
|-------------------------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:------------------------------------------------------------|
| **`SHELL`**                    | Path for the [shell](https://pubs.opengroup.org/onlinepubs/9699919799/utilities/V3_chap02.html) to be used by the build. | *Auto-detected, normally `/bin/sh`*                         |
| **`PREFIX`**                   | Path for the default installation destination.                                                                                        | *`/usr/local`*                                              |
| **`DESTDIR`**                  | Path for the [staged installation directory](https://www.gnu.org/prep/standards/html_node/DESTDIR.html) for package builders.  | *(Unset)*                                                   |
| **`INSTALLSH`**                | Path for the `install` program to be used during installation.                                                                                               | *`build-aux/install-sh`*                                    |
| **`SIR_NO_SYSTEM_LOGGERS`**    | Produce a build without support for platform-specific logging facilities                                                                | *`0` (disabled; use `1` to enable)*                         |
| **`SIR_NO_PLUGINS`**           | Produce a build without support for plugins                                                                 | *`0` (disabled; use `1` to enable)*                         |
| **`SIR_DEBUG`**                | Produce a build without optimizations and with full debug symbols&mdash;not for production!                         | *`0` (disabled; use `1` to enable)*                         |
| **`SIR_ASSERT_ENABLED`**       | Produce a build that calls `assert`; needs `SIR_DEBUG=1` to be useful&mdash;not for production!  | *`0` (disabled; use `1` to enable)*                         |
| **`SIR_SELFLOG`**              | Produce a build that with internal diagnostic routines that report certain events that may aid in debugging.     | *`0` (disabled; use `1` to enable)*                         |
| **`SIR_MSVCRT_MINGW`**         | Produce a build with [MinGW-w64](https://www.mingw-w64.org/) capable of using *only* the legacy MSVCRT runtime | *`0` (disabled; use `1` to enable)*                         |
| **`MINGW`**                    | Build with the [MinGW-w64](https://www.mingw-w64.org/) toolchain                                                                                   | *Auto-detected, normally `0` (disabled; use `1` to enable)* |
| **`HAIKU`**                    | Build with the [Haiku](https://www.haiku-os.org/) toolchain                                                                                        | *Auto-detected, normally `0` (disabled; use `1` to enable)* |
| **`OPENBSD`**                  | Build with the [OpenBSD](https://www.openbsd.org/) toolchain                                                                                       | *Auto-detected, normally `0` (disabled; use `1` to enable)* |
| **`NETBSD`**                   | Build with the [NetBSD](https://www.netbsd.org/) toolchain                                                                                         | *Auto-detected, normally `0` (disabled; use `1` to enable)* |
| **`SUNPRO`**                   | Build with the [Oracle Developer Studio](https://www.oracle.com/application-development/developerstudio/) toolchain                                | *Auto-detected, normally `0` (disabled; use `1` to enable)* |
| **`INTELC`**                   | Build with the [Intel C++ Compiler Classic](https://en.wikipedia.org/wiki/Intel_C%2B%2B_Compiler) toolchain                               | *Auto-detected, normally `0` (disabled; use `1` to enable)* |
| **`NVIDIAC`**                  | Build with the [NVIDIA HPC SDK](https://developer.nvidia.com/hpc-sdk) (*or the legacy Portland Group PGI C/C++*) toolchain                         | *Auto-detected, normally `0` (disabled; use `1` to enable)* |
| **`IBMXLC`**                   | Build with the [IBM XL C/C++ for AIX V16.1](https://www.ibm.com/docs/en/xl-c-and-cpp-aix/16.1) toolchain                                        | *Auto-detected, normally `0` (disabled; use `1` to enable)* |
