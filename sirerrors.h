...
#define SIR_E_PLATFORM _sir_mkerror(11) /**< Platform error code %d: %s */

static const struct {
    sirerror_t       e;
    const sirchar_t* msg;
} sir_errors[] = {{SIR_E_NOERROR, "The operation completed successfully"},
    {SIR_E_NOTREADY, "SIR has not been initialized"}, {SIR_E_ALREADY, "SIR is already initialized"},
    {SIR_E_DUPFILE, "File already managed by SIR"}, {SIR_E_NOFILE, "File not managed by SIR"},
    {SIR_E_FCFULL, "Maximum number of files already managed"}, {SIR_E_OPTIONS, "Option flags are invalid"},
    {SIR_E_LEVELS, "Level flags are invalid"}, {SIR_E_TEXTSTYLE, "Text style is invalid"},
    {SIR_E_STRING, "Invalid string argument"}, {SIR_E_NODEST, "No destinations registered for level"},
    {SIR_E_PLATFORM, "Platform error code %d: %s"}};
